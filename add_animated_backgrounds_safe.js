...
for (const [file, img] of Object.entries(bgs)) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('{/* ── Neutral Apple-style silver wisps (no blue tint) ── */}') ||
            lines[i].includes('{/* ── Neutral silver wisps (no blue) ── */}')) {
            startIdx = i;
            ...
            for (let j = i + 1; j < lines.length; j++) {
                ...
                // For this specific block, we know it's a div containing exactly 3 <div ... /> tags
                // Let's just hardcode finding the end of the block.
                if (started && lines[j].includes('</div>')) {
                    // check if the line before has 'orb3' or 'orb-drift-3'
                    if (lines[j - 1].includes('orb-drift-3') || lines[j - 1].includes('orb3') || lines[j - 1].includes('animation: \'orb')) {
                        endIdx = j;
                        break;
                    }
                }
